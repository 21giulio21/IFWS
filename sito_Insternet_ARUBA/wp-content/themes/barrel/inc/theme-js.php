<?php
	...
	add_action( 'wp_enqueue_scripts', 'barrel_enqueue_dynamic_js', '999' );

	if (!function_exists('barrel_enqueue_dynamic_js')) :
	function barrel_enqueue_dynamic_js( ) {
	...
	if (!function_exists('barrel_get_js')) :
	function barrel_get_js () {
		$barrel_theme_options = barrel_get_theme_options();
		// ===
		ob_start();
    ?>
    ...
    $(document).ready(function() {
        ...
        <?php if(isset($barrel_theme_options['custom_js_code'])) { 
        	echo ($barrel_theme_options['custom_js_code']); // This variable contains user Custom JS code and can't be escaped with WordPress functions
        } 
        ?>
