    ...
    class IPANEL
    {
        ...
        public function enqueue()
        {
            if ((isset($_GET['page'])) && ('ipanel_' . $this->args['ID'] == $_GET['page'])) {
                $assets_url = BARREL_IPANEL_URI . 'assets/';
                ...
                wp_localize_script(
                    ...
                    array(
                        ...
                        'reset_confirm_message' => esc_html__("Are you sure you want to reset back to the defaults?", 'barrel'),
                        'confirm_message' => esc_html__("Are you sure?", 'barrel'),
        ...
        function saveOptions($data, $panel_id)
        {
            ...
            $output['message'] = $output['status'] === 'error' ? esc_html__('Error!', 'barrel') : esc_html__('Options Updated.', 'barrel');
            $output['test'] = ($data);
        ...
        function ajaxRequestsHandle()
        {
            ...
            if ($_nonce === false) {
                die(
                    json_encode(
                        array(
                        'status' => 'error',
                        'message' => esc_html__('Error!', 'barrel')
                        )
                    )
                );
            }
            if ($_POST['action2'] == 'save_settings') {
                ...
                if ($_POST['import_type'] == 'code') { // Check if import data is code
                    ...
                    die(
                        json_decode(
                            array(
                            'status' => 'error',
                            'message' => esc_html__("You've not specified any data.", 'barrel')
                            )
                        )
                    );
                }
                ...
                if ($add_option === true) { // Check if add data was successfuly
                    ...
                    $output['message'] = esc_html__('Data has been imported successfully.', 'barrel');
                } else { // and if not
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('An error occured while importing data.', 'barrel');
                }
                ...
                $output['message'] = $delete === false ? esc_html__('Error!', 'barrel') : esc_html__('Options Reset.', 'barrel');
                $this->saveAllOptions();
                ...
                if (@unlink(urldecode($_POST['file_path']))) {
                    $output['status']  = 'succeed';
                    $output['message'] = esc_html__('The file permanently deleted.', 'barrel');
                } else {
                    $output['status']  = 'error';
                    $output['message'] = esc_html__('ERROR!', 'barrel');
                }
                echo json_encode($output);
                ...
                if (current_user_can('upload_files')) {
                    ...
                    if (array_key_exists('error', $movefile)) {
                        ...
                        if ($is_image) {
                            ...
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'barrel').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'barrel').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
                            ...
                            $HTMLRESULTS = '    <img class="ipanel-qup-up-img" src="' . BARREL_IPANEL_URI . 'assets/img/document.png" />' . "\n";
                            $HTMLRESULTS .= '    <div class="ipanel_file_info">' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File URL', 'barrel').': ' . '<code>' . $movefile['url'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    '.esc_html__('File Path', 'barrel').': ' . '<code>' . $movefile['file'] . '</code><br/><br/>' . "\n";
                            $HTMLRESULTS .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $movefile['file'] . '">Delete</a>' . "\n";
        ...
        public function addOptions()
        {
            if (get_option($this->args['ID']) === false) {
                if (defined('BARREL_IPANEL_PLUGIN_USAGE') && BARREL_IPANEL_PLUGIN_USAGE !== true) {
        ...
        public function downloadExport()
        {
            if (isset($_GET['action']) && $_GET['action'] == 'ipanel-download-export-file') {
                ...
                if ($data === false) {
                    wp_die(esc_html__('Error in download the file.', 'barrel'));
                }
        ...
        public function menuPage()
        {
            ...
            $theme_key_var = 'barrel_license_key_status';
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_complete') {
              update_option($theme_key_var, 'activated');
              delete_option('barrel_license_skip_date');
            }
            ...
            if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'registration_skip') {
              update_option('barrel_license_skip_date', strtotime("+1 day"));
              update_option($theme_key_var, 'activated');
            }

            if(strtotime("now") > get_option( 'barrel_license_skip_date', 0 ) && get_option( 'barrel_license_skip_date', 0 ) > 0) {
              delete_option('barrel_license_skip_date');
              delete_option($theme_key_var);
            }
            ...
            <div class="ipanel-form-stuff" style="margin-top: 20px;">
                ...
                <button class="button button-primary ipanel-save-settings"><?php echo esc_html__('Save All Changes', 'barrel'); ?></button>
               ...
               <a class="button button-primary ipanel-more-themes" target="_blank" href="http://magniumthemes.com/themes/"><?php echo esc_html__('Premium WordPress Themes', 'barrel'); ?></a>
                <a class="button button-secondary ipanel-more-themes" target="_blank" href="http://support.magniumthemes.com/"><?php echo esc_html__('Support Center', 'barrel'); ?></a>
                <?php if ($livePreview) {
                        echo '<button class="button button-secondary ipanel-toggle-preview"><i class="ipanel-black-icon eye"></i> '.esc_html__('Toggle Preview', 'barrel').'</button>'; 
                    } 
                ?>
                </div>
                <div class="clr"></div>
                    ...
                    <tr>
                            ...
                            <!-- Left Side-->
                                ...
                                <div id="ipanel-tabs">
                                    <ul>
                                        ...
                                        ?>
                                        <li id="tab-registration"><a href="#" data-target="registration"><?php esc_html_e('Theme Registration', 'barrel'); ?></a></li>
                                        <?php endif; ?>
                        ...
                        <td id="ipanel-fields-container">
                            <div>
                                ...
                                <form id="theme-activation-form" action="themes.php?page=ipanel_BARREL_PANEL&act=activation" method="POST">
                                <div class="theme-activation-wrapper">
                                  <h1><?php esc_html_e('Welcome to Barrel!', 'barrel'); ?></h1>
                                  <p class="text-large"><?php echo wp_kses_post('Barrel is now installed and ready to use! Please register your purchase to get themes updates notifications,<br>import theme demos and get access to premium dedicated support. We hope you enjoy it!', 'barrel'); ?></p>
                                  <input required name="email" type="email" class="activate-theme-email" placeholder="<?php esc_html_e('Enter your email', 'barrel'); ?>"/><br/><input name="code" type="text" placeholder="<?php esc_html_e('Enter your theme purchase code', 'barrel'); ?>" class="activate-theme-code"/>
                                  <a href="http://magniumthemes.com/go/how-to-find-purchase-code/" target="_blank" class="activate-theme-subblink"><?php esc_html_e('How to find your theme purchase code?', 'barrel'); ?></a><br>
                                  <input type="checkbox" checked name="subscribe" id="subscribe" class="activate-theme-subscribe"> <label class="radio-inline" for="subscribe"><?php esc_html_e('Receive theme updates notifications and news', 'barrel'); ?></label><br>
                                  <br/><input required class="button button-primary button-hero activate-theme-btn" type="button" value="<?php esc_html_e('Register theme', 'barrel'); ?>"/>
                                  <div class="theme-activation-message"></div>
                                  <p class="text-grey"><br><?php esc_html_e('By using this form you confirm that your purchase code, site url and email address will be saved on our servers for theme license verification purposes.', 'barrel'); ?></p>
            ...
            <div id="ipanel-preview-wrap" style="display:none;">
                ...
                <div class="imac ipanel-device" id="ipanel-preview-desktop">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="1010" height="573"><?php echo esc_html__("Your browser doesn't support iframes.", 'barrel'); ?></iframe>
                </div>
                
                <div class="ipad ipanel-device" id="ipanel-preview-tablet">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="767" height="1037"><?php echo esc_html__("Your browser doesn't support iframes.", 'barrel'); ?></iframe>
                </div>
                
                <div class="iphone ipanel-device" id="ipanel-preview-phone">
                    <iframe src="<?php echo esc_url($this->args['live_preview']); ?>" frameborder="0" width="642" height="1127"><?php echo esc_html__("Your browser doesn't support iframes.", 'barrel'); ?></iframe>
                </div>
            ...
            <div class="ipanel-form-stuff">
                ...
                <button class="button button-secondary ipanel-reset-settings"><i class="ipanel-black-icon trash"></i> <?php echo esc_html__('Reset theme options', 'barrel'); ?></button>
        ...
        public function quickUpload($options = null, $Repeater = array())
        {
            ...
            if (!empty($options['field_options']['btn_text'])) {
                ...
                $btn_text = esc_html__("Choose File", 'barrel');
            }
            ...
            if (filter_var($value['url'], FILTER_VALIDATE_URL) !== false) {
                ...
                if ($is_image) {
                    ...
                    $input .= '    '.esc_html__('File URL', 'barrel').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'barrel').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
                    $input .= '    <a href="#" class="ipanel-remove-qup-file" data-filePath="' . $value['path'] . '">'.esc_html__('Delete', 'barrel').'</a>' . "\n";
                    $input .= '    </div>' . "\n";
                    $input .= '    <div class="clr"></div></div>' . "\n";
                } else {
                    // File Box for Other File Types
                    $input .= '    <div class="ipanel-quick-upload-file">' . "\n";
                    $input .= '    <img class="ipanel-qup-up-img" src="' . BARREL_IPANEL_URI . 'assets/img/document.png" />' . "\n";
                    $input .= '    <div class="ipanel_file_info">' . "\n";
                    $input .= '    '.esc_html__('File URL', 'barrel').': ' . '<code>' . $value['url'] . '</code><br/><br/>' . "\n";
                    $input .= '    '.esc_html__('File Path', 'barrel').': ' . '<code>' . $value['path'] . '</code><br/><br/>' . "\n";
        ...
        public function repeater($options = null)
        {
            ...
            if (count($value)) { // If repeater has any item
                foreach ($value as $itemKey => $itemVal) {
                    ...
                    $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'barrel') . '<span class="item_number"> ' . ($i + 1) . '</span>' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '<span class="item_number">' . $i + 1 . '</span>') . '</h2>';
                ...
                $inputs .= '<i class="ipanel-delete-icon ipanel-delete-repeater-item"></i>';
                $inputs .= '<i class="ipanel-drag-icon ipanel-drag-repeater-item"></i>';
                $inputs .= empty($options['field_options']['each_title']) ? '<h2 class="ipanel-repeater-title">' . esc_html__("Item", 'barrel') . ' 1' . '</h2>' : '<h2 class="ipanel-repeater-title">' . sprintf($options['field_options']['each_title'], '1') . '</h2>';
                $inputs .= '<div class="ipanel-repeater-fields">';
            ...
            $inputs .= '<button class="ipanel-button ipanel-button-white ipanel-repeater-new-item">'.esc_html__("New Item", 'barrel').'</button>';
        ...
        {
            $button_text  = isset($options['field_options']['button_text']) ? $options['field_options']['button_text'] : esc_html__("Upload", 'barrel');
            $media_title  = isset($options['field_options']['media_title']) ? $options['field_options']['media_title'] : esc_html__("Choose File", 'barrel');
            $media_button = isset($options['field_options']['media_insert_button_text']) ? $options['field_options']['media_insert_button_text'] : esc_html__("Choose File", 'barrel');
            if (isset($options['repeater_item']) && $options['repeater_item'] === true) {
                $option_name = $Repeater['id'] . '[' . $Repeater['pos'] . '][' . $options['id'] . ']';
            } else {
                $option_name = $options['id'];
            ...
            $input .= '    <input class="ipanel-button ipanel-button-white ipanel-media-remove-button" type="button" value="' . esc_html__("Remove", 'barrel') . '"/>';
        ...
        public function export($options = null)
        {
            ...
            if ($export_value === false) {
                $export_value = esc_html__("You need to save options first.", 'barrel');
            }
            $op = '';
            ...
            $op .= '<a href="' . esc_url($url) . '" class="ipanel-button ipanel-button-white ipanel-export-link-btn" target="_blank"> ' . esc_html__('Download Theme Options Backup File', 'barrel') . '</a>';
            $op .= '</div>';
        ...
        public function import($options = null)
        {
            ...
            $op .= '<label><p>' . esc_html__('Paste exported theme options string to import options from backup:', 'barrel') . '</p><textarea class="ipanel-import-text"></textarea></label>';
            $op .= '<br/><br/>' . esc_html__('OR import from file:', 'barrel') . '<br/><br/><label class="ipanel-import-file-label ipanel-button ipanel-button-white"> ' . esc_html__('Choose Theme Options Backup File', 'barrel') . ' <input type="file" name="ipanel-import-file" class="ipanel-import-file"/></label>';
            $op .= '<span class="ipanel-chosen-filename"></span>';
            $op .= '<br/><br/><br/><a class="ipanel-button ipanel-button-white ipanel-import-button">'.esc_html__('Import options', 'barrel').'</a><br/><br/>';
            $op .= '</div>';
            return $this->_fieldSection($options, $op);
        }
