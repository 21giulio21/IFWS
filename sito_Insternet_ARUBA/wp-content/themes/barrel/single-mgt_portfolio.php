...
$portfolio_large = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), 'barrel-portfolio-large' );
...
$portfolio_images_sources = barrel_cmb2_get_images_src( get_the_ID(), '_barrel_portfolio_images_file_list', 'source' );
...
if($portfolio_sidebarposition == "0") {
  $portfolio_sidebarposition = $barrel_theme_options['portfolio_sidebar_position'];
}
...
if($portfolio_layout == 0) {
  ...
  $portfolio_images = barrel_cmb2_get_images_src( get_the_ID(), '_barrel_portfolio_images_file_list', 'mgt-portfolio-small' );
}
else {
	if(is_active_sidebar( 'portfolio-sidebar' ) && ($portfolio_sidebarposition <> 'disable') ) {
		$span_class = 'col-md-9';
  ...
  $portfolio_images = barrel_cmb2_get_images_src( get_the_ID(), '_barrel_portfolio_images_file_list', 'barrel-portfolio-large' );
}

// full image size for fullwidth slider
if($portfolio_fullwidthslider) { 

  $portfolio_images = barrel_cmb2_get_images_src( get_the_ID(), '_barrel_portfolio_images_file_list', 'source' );
...
barrel_set_theme_data($theme_data);

// PORTFOLIO ITEM DETAILS END

// TITLE SETTINGS
$header_background_image = get_post_meta( get_the_ID(), '_barrel_header_image', true );
...
$header_background_color =  get_post_meta( get_the_ID(), '_barrel_header_bgcolor', true );
...
?>
<?php if(isset($barrel_theme_options['portfolio_show_item_navigation']) && $barrel_theme_options['portfolio_show_item_navigation']): ?>
    <?php
        $prevPost = get_previous_post();

        if($prevPost) {
          $prevthumbnail = get_the_post_thumbnail($prevPost->ID, 'barrel-portfolio-nav');
        } else {
          $prevthumbnail = false;
        }
        ...
        if($nextPost) {
          $nextthumbnail = get_the_post_thumbnail($nextPost->ID, 'barrel-portfolio-nav');
        } else {
          $nextthumbnail = false;
        }
    ?>
    ...
    <div class="portfolio-navigation-prev" data-name="<?php esc_attr_e('Prev', 'barrel'); ?>">
    ...
    <div class="portfolio-navigation-next" data-name="<?php esc_attr_e('Next', 'barrel'); ?>">
      <div class="portfolio-navigation-image">
...
<div class="content-block"<?php if($page_bgcolor_css<>'') { echo ' data-style="'.esc_attr($page_bgcolor_css).'"'; }; ?>>
  <?php if($portfolio_titleposition == 'default'): ?>
    <div class="container-bg <?php echo esc_attr($page_title_layout_class); ?> <?php echo esc_attr($header_background_class); ?>" data-style="<?php echo esc_attr($header_background_image_style); ?>">
      ...
      <?php if(!$portfolio_hide_breadcrumbs) { barrel_show_breadcrumbs(); } ?>
    </div>
            ...
            <div class="row">
                ...
                <?php if(!$portfolio_hide_details): ?>
                  ...
                  <div class="project-details">
                      ...
                      <p><span><?php esc_html_e('Client:', 'barrel');?></span> <?php echo esc_html($portfolio_brand); ?></p>
                      <?php endif; ?>
                      <?php if(isset($portfolio_url) && $portfolio_url <> ''): ?>
                      <p><span><?php esc_html_e('Project url:', 'barrel');?></span> <a href="<?php echo esc_url($portfolio_url); ?>" target="_blank"><?php echo esc_html($portfolio_url); ?></a></p>
                      <?php endif; ?>
                      <p><span><?php esc_html_e('Category:', 'barrel');?></span> <?php echo esc_html($categories_html); ?></p>
                ...
                <?php if(!$portfolio_socialshare_disable): ?>
                  <?php do_action('barrel_social_share'); // this action called from plugin ?>
                <?php endif; ?>
...
<?php 
if(isset($barrel_theme_options['portfolio_show_slider_navigation']) && $barrel_theme_options['portfolio_show_slider_navigation']) {
  $portfolio_show_slider_navigation = 'true';
} else {
  $portfolio_show_slider_navigation = 'false';
}

if(isset($barrel_theme_options['portfolio_slider_autoplay']) && $barrel_theme_options['portfolio_slider_autoplay']) {
  $portfolio_slider_autoplay = 'true';
} else {
  $portfolio_slider_autoplay = 'false';
}

if(isset($barrel_theme_options['portfolio_show_slider_pagination']) && $barrel_theme_options['portfolio_show_slider_pagination']) {
...
<?php 
  wp_add_inline_script( 'barrel-script', '(function($){
...
<?php if(isset($barrel_theme_options['portfolio_related_works']) && ($barrel_theme_options['portfolio_related_works'])): ?>
