...
if(isset($barrel_theme_options['blog_post_hide_featured_image'])) {
	$blog_post_hide_featured_image = $barrel_theme_options['blog_post_hide_featured_image'];
} else {
...
if ( defined('DEMO_MODE') && isset($_GET['post_sidebar_position']) ) {
  $barrel_theme_options['post_sidebar_position'] = $_GET['post_sidebar_position'];
}
...
if($post_sidebarposition == "0") {
	$post_sidebarposition = $barrel_theme_options['post_sidebar_position'];
}
...
if(isset($barrel_theme_options['blog_post_title'])) {
	$blog_post_title = $barrel_theme_options['blog_post_title'];
} else {
...
<?php if ( is_active_sidebar( 'main-sidebar' ) && ( $post_sidebarposition == 'left')) : ?>
		...
		<div class="<?php echo esc_attr($span_class); ?>">
			<div class="blog-post blog-post-single<?php echo esc_attr($add_class); ?>">
					...
					<div class="post-content-wrapper">
						...
						<div class="post-content">
							...
							<div class="entry-content">
								<?php the_content('<div class="more-link">'.esc_html__( 'Continue reading...', 'barrel' ).'</div>' ); ?>
								<?php
									wp_link_pages( array(
				...
				?>
				<span class="tags">
				<?php printf( wp_kses_post(__('Tags:&nbsp; %1$s', 'barrel' )), $tags_list ); ?>
				</span>
				<?php endif; // End if $tags_list ?>
				
				<div class="post-info">
				<span><?php the_time(get_option( 'date_format' ));  ?></span><span class="post-comments-count"><i class="fa fa-comment"></i><?php echo esc_html($post_comments); ?></span><?php edit_post_link( esc_html__( 'Edit', 'barrel' ), ' <span class="edit-link">', '</span>' ); ?>
				</div>
			...
			<?php if(isset($barrel_theme_options['blog_enable_author_info'])&&($barrel_theme_options['blog_enable_author_info'])): ?>
				<?php if ( is_single() && get_the_author_meta( 'description' ) ) : ?>
					<?php get_template_part( 'author-bio' ); ?>
				<?php endif; ?>
			<?php endif; ?>
			<?php 
			if(isset($barrel_theme_options['blog_post_navigation']) && $barrel_theme_options['blog_post_navigation']) {
				barrel_content_nav( 'nav-below' ); 
			}
			?>
			<?php
