...
function mgt_shortcode_post_list_wp($atts, $content = null) {
	...
	foreach($posts as $post) {
		...
		if($showreadmore == 1) {
			$readmore_html = '<a href="'.get_permalink($post->ID).'" class="btn mgt-button mgt-style-borderedgrey hvr-'.$hover_effect.' mgt-size-small mgt-align-left mgt-display-inline mgt-text-size-normal mgt-text-transform-none">'.esc_html__('Read more', 'barrel').'</a>';
		} else {
			$readmore_html = '';
		}
		...
		echo '<div class="mgt-post-details"><div class="mgt-post-icon '.esc_attr($post_format_class).'"></div><div class="mgt-post-date">
'.get_the_time( get_option( 'date_format' ), $post->ID ).' '.wp_kses_post($comments_html).'</div><div class="mgt-post-title"><a href="'.get_permalink($post->ID).'"><h5>'.esc_html($post->post_title).'</h5></a></div>'.wp_kses_post($exrcept_html).wp_kses_post($readmore_html).'</div>';
