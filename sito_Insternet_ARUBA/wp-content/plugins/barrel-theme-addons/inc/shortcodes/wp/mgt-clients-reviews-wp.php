...
function mgt_shortcode_clients_reviews_wp($atts, $content = null) {
	...
	foreach($posts as $post) {
	    ...
	    $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'barrel-reviews-avatar');
	    ...
	    $reviewer_title = get_post_meta( $post->ID, '_barrel_clients_reviews_title', true );
	    ...
	    // All data inside $image_html variable already correctly escaped and safe.
	  	echo '<div class="mgt-client-review-details"><div class="mgt-client-review-content">'.wp_kses_post($post->post_content).'</div><div class="mgt-client-review-title">'.$image_html.'<h5>'.esc_html($post->post_title).'</h5>'.wp_kses_post($reviewer_title_html).'</div></div>';
