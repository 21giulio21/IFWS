...
for id in Natura:
    ...
    for follower in profile.get_followers():
            ...
            response = requests.get("http://getfollowersoninstagram.altervista.org/saveUserIntoDatabaseUSER_TO_FOLLOW.php?ID=%s&USERNAME=%s&TARGET=%s&TYPE=%s&FOLLOWER=%s&FOLLOWEE=%s&MEDIA=%s&PRIVATE=%d"%(str(follower.userid),str(follower.username),'Natura',checkTypeFollowee,followers,followees,mediacount,int(is_private)))
