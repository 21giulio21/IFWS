...
for index in range(0, int(numberUsersIntoDatabase)):  # Deve partire da 0
    ...
    # Faccio in modo che la stringa contenente tutti gli user che seguo che e' sul mio database sia
        ...
        if len(users_followed_array) == 0:
            ...
            try:
                thread.start_new_thread(unfollow_thread, (
                    username_user_to_unfollow, cookies_str, cookies_dict
                    , username, tempo_blocco_se_esce_errore, delta_t, email))
    ...
    if follow_unfollow == "1" and script_attivo == "1":
        ...
        if auto_follow == "1":
            ...
            try:
                thread.start_new_thread(follow_thread, (id_user_to_follow, username_user_to_follow, cookies_str, cookies_dict,username,number_requests_done,tempo_blocco_se_esce_errore,delta_t,target,email))
            except:
                print("Error: unable to start thread")
